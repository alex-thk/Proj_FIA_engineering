...
group_names = list(ablative_groups.keys())

# Iteriamo sulle combinazioni da 2 fino al massimo numero di gruppi
for r in range(2, len(ablative_groups) + 1):
    # Genera tutte le combinazioni di gruppi di numerosità 'r'
    for combination in itertools.combinations(group_names, r):
        # Otteniamo le feature da includere per questa combinazione
        columns_to_include = []
        for group in combination:
            columns_to_include.extend(ablative_groups[group])

        # Manteniamo sempre le colonne 'semester' e 'year'
        columns_to_include += ['semester', 'year']


        # Verifica se tutte le colonne esistono nel dataset
        columns_to_include = [col for col in columns_to_include if col in dataset_originale.columns]

        print(f'Colonne che effettivamente si usano per questa iterazione: {columns_to_include}')

        # **Crea una copia del dataset con le colonne selezionate** per evitare modifiche dirette
        df = dataset_originale[columns_to_include].copy()
        ...
        performance[combination_name] = best_score

        print(f"Combinazione: {combination_name}, Performance: {best_score:.4f}")

# Convertiamo i risultati in un DataFrame per analizzarli
results_df = pd.DataFrame(list(performance.items()), columns=['Combination', 'Performance'])
